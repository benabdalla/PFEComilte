...
export class TimelineComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // Subscribe to route parameter changes to handle tag navigation
    this.subscriptions.push(
      this.activatedRoute.paramMap.subscribe(params => {
        const tag = params.get('tagName');
          ...
          next: (response: PostResponse[] | HttpErrorResponse) => {
            ...
            if (Array.isArray(response)) {
              if (response.length === 0 && currentPage === 1) {
                ...
                this.noPost = false;
                if (currentPage === 1) {
                  this.timelinePostResponseList = response;
                } else {
                  response.forEach(pR => this.timelinePostResponseList.push(pR));
                }
                this.hasMoreResult = response.length === this.resultSize;
                this.resultPage++;
