...
export class AbsenceService {
  ...
  /**
   * Update absence justification and change status to justified (text only)
   */
  updateAbsenceJustification(id: number, data: { justificationText?: string, justificationFile?: string }): Observable<Absence> {
    // Include status change to 'JUSTIFIED' when submitting justification
    const updateData = {
      ...data,
      status: 'JUSTIFIED'
    };
    return this.http.patch<Absence>(`${this.apiUrl}/${id}/justify`, updateData);
  }

  /**
   * NEW: Update absence justification with file uploads and description
   * This method handles both description and multiple file uploads
   */
  updateAbsenceJustificationWithFiles(id: number, justificationData: {
    description: string;
    startDate?: string;
    endDate?: string;
    isRecurring?: boolean;
    ...
    if (justificationData.startDate) {
      formData.append('startDate', justificationData.startDate);
    }
    if (justificationData.endDate) {
      formData.append('endDate', justificationData.endDate);
    }
    formData.append('isRecurring', justificationData.isRecurring ? 'true' : 'false');
    
    // Add files
    justificationData.files.forEach((file, index) => {
      formData.append('files', file, file.name);
    });
    
    return this.http.post<Absence>(`${this.apiUrl}/${id}/justify-with-files`, formData).pipe(
      catchError((error: any) => {
        console.error('Error in updateAbsenceJustificationWithFiles:', error);
        throw error;
      })
    );
  }
  ...
  /**
   * NEW: Upload files for an absence (separate endpoint)
   */
  uploadFilesForAbsence(absenceId: number, files: File[]): Observable<any[]> {
    const formData = new FormData();
    files.forEach((file) => {
      formData.append('files', file, file.name);
    });
    
    return this.http.post<any[]>(`${this.apiUrl}/${absenceId}/files`, formData);
  }

  /**
   * NEW: Get files for an absence
   */
  getAbsenceFiles(absenceId: number): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/${absenceId}/files`);
  }

  /**
   * NEW: Download a specific file
   */
  downloadAbsenceFile(absenceId: number, fileId: number): Observable<Blob> {
    return this.http.get(`${this.apiUrl}/${absenceId}/files/${fileId}/download`, {
      responseType: 'blob'
    });
  }
   ...
   */
  deleteAbsenceFile(absenceId: number, fileId: number): Observable<void> {
    return this.http.delete<void>(`${this.apiUrl}/${absenceId}/files/${fileId}`);
  }
