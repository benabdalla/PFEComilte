...
export class AbsenceNotificationComponent implements OnInit, OnDestroy {
  ...
  private initializeForm(): void {
    this.justificationForm = this.formBuilder.group({
      type: [''],
      startDate: [this.selectedAbsence?.date || new Date().toISOString().split('T')[0]],
      endDate: [this.selectedAbsence?.date || new Date().toISOString().split('T')[0]],
      isRecurring: [false],
      reason: [this.selectedAbsence?.justificationText || '', [Validators.required, Validators.minLength(10), Validators.maxLength(500)]],
      contactInfo: [''],
      hasFiles: [false, Validators.requiredTrue] // Custom validator for file requirement
    });
  }
  ...
  get hasFilesControl() { return this.justificationForm.get('hasFiles'); }

  // Method to update file validation
  private updateFileValidation(): void {
    const hasFiles = this.attachedFiles.length > 0;
    this.justificationForm.patchValue({ hasFiles });
  }
  ...
  onSubmit(): void {
    console.log('Form submitted with values:', this.justificationForm.value);
    if (this.justificationForm.valid && this.selectedAbsence?.id) {
      ...
      const reasonControl = this.justificationForm.get('reason');
      const hasFilesControl = this.justificationForm.get('hasFiles');

      if (!reasonControl?.valid) {
        this.snackBar.open('Please provide a detailed explanation (minimum 10 characters).', 'Close', {
          duration: 3000,
          panelClass: ['warning-snackbar']
        });
        return;
      ...
      if (!hasFilesControl?.valid || this.attachedFiles.length === 0) {
        this.snackBar.open('Please upload at least one supporting document.', 'Close', {
          duration: 3000,
          panelClass: ['warning-snackbar']
        });
        return;
  ...
  private submitJustification(): void {
    ...
    const justificationData = {
      ...
      files: this.attachedFiles.map(attachedFile => attachedFile.file)
    };

    console.log('Submitting justification data:', justificationData);

    // Use the new API that handles both description and files
    const subscription = this.absenceService.updateAbsenceJustificationWithFiles(
      this.selectedAbsence.id,
      justificationData
    ).subscribe({
      ...
      error: (error: any) => {
        console.error('Error submitting justification with files:', error);
  ...
  private handleSuccessfulSubmission(updatedAbsence: Absence): void {
    // Check if status was successfully changed to justified
    const fileCount = this.attachedFiles.length;
    const fileText = fileCount > 0 ? ` with ${fileCount} file(s)` : '';
    const statusMessage = updatedAbsence.status === 'JUSTIFIED'
      ? `Justification submitted successfully${fileText}! Status changed to JUSTIFIED.`
      : `Justification submitted successfully${fileText}!`;
